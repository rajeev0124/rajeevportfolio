...
(function ($) {
  ...
  ////////////////////////////////////////////////////
  // 16. Originkit Scramble & Hover Glitch Reveal for "developer"
  $(document).ready(function () {
    const $target = $(".banner-three-title");
    if (!$target.length) return;

    const originalText = $target.text().trim() || "developer";
    const GLITCH_CHARS_UPPER = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    const GLITCH_CHARS_LOWER = "abcdefghijklmnopqrstuvwxyz";
    const GLITCH_ALL = "abcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*";
    ...
    // ── 1. Enter Scramble Animation ───────────────────────
    let isEntering = true;
    const scrambleDuration = 1800; // ms
    ...
    const enterInterval = setInterval(() => {
      ...
      // Left-to-right sequential locking with easing
      const easeProgress = 1 - Math.pow(1 - progress, 2);
      const lockedCount = Math.floor(easeProgress * chars.length);

      charSpans.forEach(($span, idx) => {
        const original = chars[idx];
        if (idx < lockedCount || progress >= 1) {
          $span.text(original);
        } else {
          // Glitch / Scramble active
          $span.text(nextGlitchChar(original));
        }
      });
      ...
      if (progress >= 1) {
        ...
        isEntering = false;
        charSpans.forEach(($span, idx) => $span.text(chars[idx]));
      }
    }, 45);

    // ── 2. Interactive Hover Diffusion & Wave Glitch ──────
    let activeWaveRaf = null;
    ...
    $target.on("mousemove", function (e) {
      ...
      const hoverIndexRatio = Math.max(0, Math.min(1, mouseX / targetWidth));
      const activeIdx = Math.floor(hoverIndexRatio * chars.length);
      ...
      charSpans.forEach(($span, idx) => {
        ...
        if (dist === 0) {
          ...
          // Diffusion radius
          const rndChar = GLITCH_ALL[Math.floor(Math.random() * GLITCH_ALL.length)];
          $span.text(rndChar);
        } else {
          $span.text(original);
        }
    ...
    $target.on("mouseleave", function () {
      if (isEntering) return;
      // Smooth collapse back to original
      let step = 0;
